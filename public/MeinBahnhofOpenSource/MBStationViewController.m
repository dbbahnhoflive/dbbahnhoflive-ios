...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[MBStationTeaserWithImageCollectionViewCell class] forCellWithReuseIdentifier:@"TeaserImage"];
    ...
    self.tafelContainerView.layer.shadowColor = [[UIColor db_dadada] CGColor];
...
-(void)updateContainerFrames{
    ...
    if(self.station.newsList.count > 0){
        ...
        tafelY = CGRectGetMaxY(newsFrame)+5;
    } else {
...
- (void)reloadStationData {
    ...
    BOOL hasPXRShops = _station.riPoiCategories.count > 0;
    BOOL hasEinkaufShops = self.station.einkaufsbahnhofCategories.count > 0;
    BOOL hasShop = hasPXRShops || hasEinkaufShops;
    if (hasShop && hasOPNV) {
        [secondRowKacheln addObject:self.shopsKachel];
        [secondRowKacheln addObject:karteKachel];
    } else if(!hasShop && hasOPNV){
        [secondRowKacheln addObject:karteKachel];
    } else if(!hasShop && !hasOPNV){
        //second row is empty
    } else {
    ...
    if (nil != _station.facilityStatusPOIs && _station.facilityStatusPOIs.count > 0) {
        for (FacilityStatus *poi in _station.facilityStatusPOIs) {
            if (poi.type == ELEVATOR && (poi.state == UNKNOWN || poi.state == INACTIVE)) {
                brokenEscalators += 1;
            }
        }
        if (nil != self.fahrstuhlKachel) {
    ...
    //construct arrays
    [_kacheln addObject:firstRowKacheln];
    if(secondRowKacheln.count > 0){
        [_kacheln addObject:secondRowKacheln];
    }
    [_kacheln addObject:thirdRowKacheln];
    if(fourthRowKacheln.count > 0){
        [_kacheln addObject:fourthRowKacheln];
    }
    
    MBStationKachel* imageKachel = nil;
    if(self.station.hasPickPack){
        imageKachel = [MBStationKachel new];
        imageKachel.isImageTeaser = YES;
        imageKachel.teaserType = MBStationTeaserTypePickPack;
        imageKachel.imageName = @"pickpack";
        imageKachel.title = @"Bequem einkaufen";
        imageKachel.titleForVoiceOver = @"Bequem einkaufen: Pick pack.";
    }
    ...
    if(self.station.hasChatbot){
        ...
        if(imageKachel){
            [_kacheln addObject:@[ chatKachel, imageKachel ]];
        } else {
            [_kacheln addObject:@[ chatKachel ]];
        }
    } else if(imageKachel) {
        [_kacheln addObject:@[ imageKachel ]];
    }
    if(self.station.isGreenStation){
        MBStationKachel* greenKachel = [MBStationKachel new];
        greenKachel.isGreenTeaser = YES;
        [_kacheln addObject:@[ greenKachel ]];
    }
    

    [self.collectionView reloadData];
    if(self.shopsKachel || self.fahrstuhlKachel){
...
-(CGSize)specialSizeKachel:(NSIndexPath*)indexPath{
    ...
    if(kachel.isGreenTeaser || kachel.isChatbotTeaser || kachel.isImageTeaser){
        if(kachel.isGreenTeaser){
            return [self greenTeaserSize];
        } else if(kachel.isChatbotTeaser) {
            //there can be another one next to the chatbot, the chatbot then uses 2/3
            if(((NSArray*)_kacheln[indexPath.section]).count == 1){
                return [self chatBotTeaserSize];
            } else {
                return [[self.kachelSizes objectForKey:@"large"] CGSizeValue];
            }
        } else if(kachel.isImageTeaser) {
            //this is always next to a chatbot or alone
            if(((NSArray*)_kacheln[indexPath.section]).count == 1){
                CGSize itemSize = [[self.kachelSizes objectForKey:@"xlarge"] CGSizeValue];
                itemSize.height = 166;//reduced height
                return itemSize;
            } else {
                return [[self.kachelSizes objectForKey:@"small"] CGSizeValue];
            }
        }
    }
    return CGSizeZero;
...
-(CGSize)greenTeaserSize{
    CGSize itemSize = [[self.kachelSizes objectForKey:@"xlarge"] CGSizeValue];
    itemSize.height = GREEN_TEASER_HEIGHT;
    return itemSize;
}
-(CGSize)chatBotTeaserSize{
    CGSize itemSize = [[self.kachelSizes objectForKey:@"xlarge"] CGSizeValue];
    return itemSize;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MBStationCollectionViewCell *cell = nil;
    if(kachel.isImageTeaser){
        MBStationTeaserWithImageCollectionViewCell* cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TeaserImage" forIndexPath:indexPath];
        [cell setKachel:kachel];
        cell.type = kachel.teaserType;
        return cell;
    } else if(isChatbotTeaser){
        ...
        cell.reducedSize = ((NSArray*)_kacheln[indexPath.section]).count > 1;
        return cell;
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:(isGreenTeaser) ? @"GreenCell" : @"NaviCell" forIndexPath:indexPath];
    }
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(kachel.isImageTeaser){
        if(kachel.teaserType == MBStationTeaserTypePickPack){
            [MBTrackingManager trackActionsWithStationInfo:@[@"h1",@"tap",@"pickpack"]];
            //link to shop page and open pickpack page
            [CATransaction begin];
            [CATransaction setCompletionBlock:^{
                [self.tabBarViewController selectViewControllerAtIndex:4];
            }];
            MBStationNavigationViewController* nav = self.tabBarViewController.viewControllers[4];
            MBServiceListCollectionViewController *listController
            = (MBServiceListCollectionViewController*)nav.visibleViewController;
            if(![listController isKindOfClass:MBServiceListCollectionViewController.class]){
                [nav popToRootViewControllerAnimated:NO];
            ...
            listController = nav.viewControllers.firstObject;
            listController.tabBarViewController = self.tabBarViewController;
            listController.openPickPackScreen = YES;
            [CATransaction commit];
            return;
