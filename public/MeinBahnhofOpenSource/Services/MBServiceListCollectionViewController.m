...
-(void)prepareForDisplay{
    ...
    //add additional items when this is the shop list (!info).
    if (![_type isEqualToString:@"info"]){
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    NSString *trackingKey = self.type;
    if ([trackingKey isEqualToString:@"shopping"]) {
        trackingKey = @"shops";
    }
    [MBTrackingManager trackStatesWithStationInfo:@[@"h3", trackingKey]];
    ...
    if(self.openChatBotScreen || self.openPickPackScreen){
        ...
        if(displayAsSingleScreen){
            ...
            UIViewController* vc = [[MBDetailViewController alloc] initWithService:service];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
        ...
        if ([menuItem.type isEqualToString:@"aufzuegeundfahrtreppen"]) {
            ...
            if (services.count > 1) {
                ...
                if([service isKindOfClass:MBService.class]){
                    vc = [[MBDetailViewController alloc] initWithService:service];
                    [(MBDetailViewController *)vc setItem:service];
                }
            }
        }
    ...
    if(vc){
        ...
        if(title.length > 0){
            NSString *trackingKey = @"info";
            if ([self.type isEqualToString:@"shopping"]) {
                trackingKey = @"shops";
            }
            [MBTrackingManager trackActionsWithStationInfo:@[@"h3", trackingKey, @"tap", title]];
